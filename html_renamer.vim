function! Rename_tag()
	...
	if target_char !=? "<" && target_char !=? ">"
		...
		if [result_line,result_col] == [cursor_pos[1],cursor_pos[2]]
			...
			if !search('/\@<=.*\%#','')
				normal %
				echom "moved to closing tag"
				let l:end_tag_pos = getcursorcharpos()
				let l:end_tag_pos[2] = l:end_tag_pos[2] + 1
			else
				let l:end_tag_pos = getcursorcharpos()
			endif
			echom "col of end tag is: " . string(l:end_tag_pos[2])
			...
			let l:first_tag_match = matchadd('rename_hi', '<\@<=\(\w*\)\%#\(\w*\)')
...
function! Update_tags(start_tag_start,end_tag_start,first_tag_match)
	...
	if a:start_tag_start[1] == a:end_tag_start[1]
		normal e
		let l:col_end_word = getcursorcharpos()[2]
		let l:word_length = (l:col_end_word - a:start_tag_start[2])
		let a:end_tag_start[2] = a:end_tag_start[2] - l:word_length
	endif
	echom a:end_tag_start
	normal diw
	...
	while 1
		...
		let l:last_match = matchaddpos('rename_hi',[[l:line,l:col,l:length]])
...
"hi rename_hi guifg=#eb844580 guibg=#e4c5af80
hi link rename_hi DiffText
